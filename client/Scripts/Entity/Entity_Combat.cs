...
public class Entity_Combat : MonoBehaviour 
{
    ...
    [Header("UI 绑定")]
    public UI_BuffManager myBuffBar;
    ...
    public void AddEffectToCurrentWeapon(WeaponEffectSO effect)
    {
        var currentWeapon = GetComponentInChildren<Weapon>();

        if (currentWeapon != null)
        {
            currentWeapon.AddEffect(effect);

            if (myBuffBar != null && effect.showInUI)
            {
                // 🌟 换成调用新的方法
                myBuffBar.AddOrRefreshBuffUI(effect);
            }
        }
    }
