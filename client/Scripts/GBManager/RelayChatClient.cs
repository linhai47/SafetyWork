...
public class RelayChatClient : MonoBehaviour
{
    ...
    /// 统一 JSON 发送入口。
    /// </summary>
    private async Task SendJson(WsMessage message)
    {
        if (websocket == null)
        {
    ...
    {
        Debug.Log($"[{clientId}] 收到服务端原始消息: {json}");

        WsMessage message = null;
        try
        {
            message = JsonUtility.FromJson<WsMessage>(json);
        }
        catch (Exception ex)
        {
            Debug.LogWarning($"[{clientId}] JSON 解析失败: {ex.Message}");
            return;
        }

        if (message == null || string.IsNullOrEmpty(message.type))
        {
            Debug.LogWarning($"[{clientId}] 收到消息缺少 type。");
            return;
        }
        ...
        switch (message.type)
        {
            case "SERVER_BROADCAST":
                Debug.Log(
                    $"[{clientId}] 转发消息: room={message.roomId}, " +
                    $"from={message.fromClientId}, text={message.text}, time={message.timestamp}"
                );
                break;
            case "ERROR":
                Debug.LogError($"[{clientId}] 服务端错误: {message.error}");
                break;
            default:
                Debug.Log($"[{clientId}] 未处理消息类型: {message.type}");
                break;
        }
    }
